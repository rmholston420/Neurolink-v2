"""Alembic environment for Neurolink-v2.

Resolves the database URL from the app settings, converting the async driver
(``sqlite+aiosqlite``) to its sync equivalent (``sqlite``) so Alembic can run
migrations with a plain synchronous engine.
"""
